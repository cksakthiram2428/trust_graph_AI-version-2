      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 p-4 rounded-2xl bg-white dark:bg-[#0E0E12] border border-slate-200 dark:border-white/10 shadow-sm transition-colors">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            aria-label="Filter suppliers by industry sector"
            title="Filter suppliers by industry sector"
            value={selectedIndustry}
            onChange={(e) => { sound.playClick(); setSelectedIndustry(e.target.value); }}
            className="flex-1 sm:flex-none px-3 py-2 rounded-xl bg-slate-50 dark:bg-[#0A0A0C] border border-slate-200 dark:border-white/10 text-[11px] sm:text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:border-sky-500 cursor-pointer font-mono"
          >
            {industries.map(ind => (
              <option key={ind} value={ind}>{ind === "All" ? "All Industries" : ind.split(" ")[0] + "..."}</option>
            ))}
          </select>

          <select
            id="risk-filter-select"
            aria-label="Filter suppliers by risk level"
          ...
          >
            {risks.map(r => (
              <option key={r} value={r}>{r === "All" ? "All Risks" : r.split(" ")[0]}</option>
            ))}
          </select>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="flex-1 px-3 py-2 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-sky-500/30 text-sky-700 dark:text-cyan-300 font-mono text-[10px] sm:text-xs font-semibold flex items-center justify-center gap-1.5 cursor-pointer transition-colors shadow-xs"
            >
            ...
            <button
              ...
              className="flex-1 px-3 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-sky-500/20 dark:hover:bg-sky-500/30 text-white dark:text-sky-300 border border-slate-800 dark:border-sky-500/40 text-[10px] sm:text-xs font-mono font-bold tracking-wider uppercase flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
            >
                ...
                <div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    <div className="p-2 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5">
                      <span className="text-[10px] text-slate-600 dark:text-slate-400 font-mono block">Payment Delay</span>
                      <span className="font-semibold text-slate-900 dark:text-zinc-200 font-mono">{supplier.paymentDelay}</span>
                    </div>
                    <div className="p-2 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5">
                      <span className="text-[10px] text-slate-600 dark:text-slate-400 font-mono block">Fulfillment</span>
                      <span className="font-semibold text-emerald-700 dark:text-emerald-400 font-mono">{supplier.deliveryReliability}</span>
                    </div>
                    <div className="p-2 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5">
                      <span className="text-[10px] text-slate-600 dark:text-slate-400 font-mono block">Quality Rate</span>
                      <span className="font-semibold text-sky-700 dark:text-cyan-300 font-mono">{supplier.qualityRate}</span>
                    </div>
                    <div className="p-2 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5">
                      <span className="text-[10px] text-slate-600 dark:text-slate-400 font-mono block">Complaints</span>
                      <span className={`font-semibold font-mono ${supplier.complaintCount > 5 ? "text-rose-700 dark:text-rose-400" : "text-slate-900 dark:text-zinc-200"}`}>
                ...
                <div className="mt-4 pt-3.5 border-t border-slate-100 dark:border-white/10 flex items-center justify-between gap-1.5">
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-zinc-300 hover:text-sky-700 dark:hover:text-[#22D3EE] text-xs transition-colors cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-white text-xs transition-colors cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-rose-50 dark:hover:bg-rose-950/50 text-slate-700 dark:text-zinc-300 hover:text-rose-700 dark:hover:text-rose-400 text-xs transition-colors cursor-pointer"
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-2.5 py-1.5 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-[11px] font-mono text-rose-700 dark:text-rose-300 flex items-center gap-1 cursor-pointer transition-colors font-semibold"
                    >
